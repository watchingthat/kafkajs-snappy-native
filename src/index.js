...
module.exports = () => ({
  ...
  async decompress(buffer) {
    ...
    const decodedBuffers = await Promise.all(encoded.map(async (encodedBuffer) => snappy.uncompress(encodedBuffer)));

    return decodedBuffers.reduce((result, decodedBuffer) => Buffer.concat([result, decodedBuffer]), Buffer.alloc(0));
  },
});
